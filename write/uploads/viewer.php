...
$PASSWORD = $admin_password;  // Set the password, to access the file manager... (optional)
...
if($PASSWORD) {
	...
	if(!isset($_SESSION['_sfm_allowed'])) {
		...
		if(isset($_POST['p']) && $_POST['p'] && sha1($t.$_POST['p']) === sha1($t.$PASSWORD)) {
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		var $link = $('<a class="name" />')
			.attr('href', data.is_dir ? '#' + encodeURIComponent(data.path) : './read_file.php/' + data.path)
			.text(data.name);
