...
$CONFIG = '{"lang":"en","error_reporting":true,"show_hidden":true,"hide_Cols":true,"calc_folder":false,"theme":"light"}';


/**
 * H3K | Tiny File Manager V2.4.7
 * CCP Programmers | ccpprogrammers@gmail.com
 * http://tinyfilemanager.github.io
 */
...
// Doc - http://www.php.net/manual/en/function.date.php
...
define('DOMAIN_TRASH_DIR', "trash");
define('DOMAIN_TRASH_DATA', "/trash/log");
                ...
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href="http://tinyfilemanager.github.io/" target="_blank" class="text-muted" data-version="<?php echo VERSION; ?>">CCP Programmers</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
defined('FM_LOG_PATH') || define('FM_LOG_PATH', getcwd().'/'.$_SESSION[FM_SESSION_ID]['logged'].".log");
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if(isset($_POST['type']) && $_POST['type']=="search") {
    	...
    	foreach ($rii as $file) {
    		...
    		if (strpos(basename($file), $findme) === false)
    			continue;
    	...
    	foreach ($files as $file_obj){
    		$file = $file_obj["file"];
    		$isdir = $file_obj["isdir"];
    		$path = ltrim(dirname($file), './');
    		$path = $path == "\\" ? "" : $path;
    		$name = ltrim(basename($file), '/');
    
    		$str .= '<li><a href="?p='.$path.($isdir ? "&checked=$path/$name" : "&view=$name").'"'.">root/".($path == "" ? "" : "$path/")."$name</a></li>";
    	}
...
//restore from trash
if (isset($_GET['restore']) && !FM_READONLY) {
    ...
    if ($rename) {
    	...
    	unset($trashItems["'$new'"]);
    	$trashItems = array_filter($trashItems);
    		
    	fm_set_msg(sprintf('Restored from trash to <b>%s</b>', fm_enc($old)));
    		
    } else if ($rename === null) {
        fm_set_msg('File or folder with this path already exists', 'alert');
    			
    }else {
    	fm_set_msg(sprintf('Error while Restore from trash to <b>%s</b>', fm_enc($old)), 'error');
    }
    	
    file_put_contents(TRASH_DATA_PATH, json_encode($trashItems), LOCK_EX);
    
    //put log file
    file_put_contents(FM_LOG_PATH, "restored trash from $from to $dest".PHP_EOL, FILE_APPEND | LOCK_EX);
    
    fm_redirect(FM_SELF_URL . '?p=' . FM_PATH);
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (FM_PATH == DOMAIN_TRASH_DIR || $_SESSION[FM_SESSION_ID]['logged'] == "admin")
		{
			if (fm_rdelete($path . '/' . $del)) {
				...
				if (isset($trashItems["'$f'"])){
					unset($trashItems["'$del'"]);
					$trashItems = array_filter($trashItems);
				}
                    
                //put log file
                file_put_contents(FM_LOG_PATH, "deleted $del from trash".PHP_EOL, FILE_APPEND | LOCK_EX);
		...
		else
		{
			...
			$newName = $oldName.'-'.date('d_m_y_H_i_s').$ext;
					
			$from = $path.'/'.$del;
			$dest = TRASH_DIR_PATH.'/'.$newName;

			$rename = fm_rename($from, $dest);
			$dir = FM_PATH == "" ? "" : FM_PATH . "/";

			if ($rename) {
				$trashItems["'$newName'"] = $dir.$del;
            
                //put log file
                file_put_contents(FM_LOG_PATH, "moved $from to trash".PHP_EOL, FILE_APPEND | LOCK_EX);
            
				fm_set_msg(sprintf('Moved <b>%s</b> to trash', fm_enc($del)));
            } else {
				fm_set_msg(sprintf('Error while moving from <b>%s</b> to trash', FM_PATH), 'error');
            }
		}
    } else {
    ...
    file_put_contents(TRASH_DATA_PATH, json_encode($trashItems));
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['new']) && isset($_GET['type']) && !FM_READONLY) {
    ...
    //put log file
    file_put_contents(FM_LOG_PATH, "created $path/$new".PHP_EOL, FILE_APPEND | LOCK_EX);
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    $dest .= '/' . pathinfo($from, PATHINFO_BASENAME);
    // move?
    $move = isset($_GET['move']);
    // copy/move/duplicate
    if ($from != $dest) {
        $msg_from = trim(FM_PATH . '/' . pathinfo($from, PATHINFO_BASENAME), '/');
       ...
       if (!$move){ //Not move and to = from so duplicate
            $msg_from = trim(FM_PATH . '/' . pathinfo($from, PATHINFO_FILENAME), '/');
     ...
     //put log file
    file_put_contents(FM_LOG_PATH, "moved=$move from $from to $dest".PHP_EOL, FILE_APPEND | LOCK_EX);
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish']) && !FM_READONLY) {
    ...
    file_put_contents(FM_LOG_PATH, "moved=$move from $path to $copy_to_path".PHP_EOL, FILE_APPEND | LOCK_EX);
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    file_put_contents(FM_LOG_PATH, "renamed from $old to $new".PHP_EOL, FILE_APPEND | LOCK_EX);
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if ( is_writable($targetPath) ) {
        ...
        if(file_exists ($fullPath) && !$override_file_name && !$chunks) {
            $ext_1 = $ext ? '.'.$ext : '';
            $fullPath = $path . '/' . $filename .'_'. date('ymdHis'). $ext_1;
        }
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
            if ($chunkTotal){
                ...
                if ($out) {
                    ...
                    //put log file
                    file_put_contents(FM_LOG_PATH, "uploaded to $fullPath".PHP_EOL, FILE_APPEND | LOCK_EX);
    ...
    if (is_array($files) && count($files)) {
        if (FM_PATH == DOMAIN_TRASH_DIR)
		{
			...
			if ($errors == 0){
				...
				//put log file
                file_put_contents(FM_LOG_PATH, "deleted ".implode(',',$files)." from trash".PHP_EOL, FILE_APPEND | LOCK_EX);
			}
			else
		...
		else
		{
			...
			if ($errors == 0){
				fm_set_msg('Selected files and folders moved to trash');
				//put log file
                file_put_contents(FM_LOG_PATH, "moved ".implode(',',$files)." to trash".PHP_EOL, FILE_APPEND | LOCK_EX);
			}
			else{
				fm_set_msg(sprintf('Error while moving items from <b>%s</b> to trash', FM_PATH), 'error');
			}
		}
    } else {
        fm_set_msg(lng('Nothing selected'), 'alert');
...
if (isset($_POST['group']) && (isset($_POST['zip']) || isset($_POST['tar'])) && !FM_READONLY) {
    ...
    if (!empty($files)) {
        ...
        if ($res) {
            //put log file
            file_put_contents(FM_LOG_PATH, "zipped ".implode(',',$files).PHP_EOL, FILE_APPEND | LOCK_EX);
...
if (isset($_GET['unzip']) && !FM_READONLY) {
    ...
    if ($isValid) {
        ...
        if ($res) {
            file_put_contents(FM_LOG_PATH, "unzipped to $path".PHP_EOL, FILE_APPEND | LOCK_EX);
...
//put log file
file_put_contents(FM_LOG_PATH, "entered $path".PHP_EOL, FILE_APPEND | LOCK_EX);
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <script src="http://cdnjs.cloudflare.com/ajax/libs/dropzone/5.5.1/min/dropzone.min.js"></script>
    <script>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="http://github.com/prasathmani/tinyfilemanager" target="_blank" class="app-v-title"> Tiny File Manager <?php echo VERSION; ?></a></h3></p>
                    ...
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="http://github.com/prasathmani/tinyfilemanager/wiki" target="_blank"><i class="fa fa-question-circle"></i> <?php echo lng('Help Documents') ?> </a> </li>
                                <li class="list-group-item"><a href="http://github.com/prasathmani/tinyfilemanager/issues" target="_blank"><i class="fa fa-bug"></i> <?php echo lng('Report Issue') ?></a></li>
        ...
        <div class="col-12">
            ...
            if($is_onlineViewer) {
                if($online_viewer == 'google') {
                    echo '<iframe src="http://docs.google.com/viewer?embedded=true&hl=en&url=' . fm_enc($file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                } else if($online_viewer == 'microsoft') {
                    echo '<iframe src="http://view.officeapps.live.com/op/embed.aspx?src=' . fm_enc($file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                }
            } elseif ($is_zip) {
                // ZIP content
                if ($filenames !== false) {
...
<form action="" method="post" class="pt-3">
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <?php if (FM_PATH == DOMAIN_TRASH_DIR): ?>
								<a title="<?php echo lng('Restore') ?>" href="#" onclick="restore('<?php echo FM_PATH?>', '<?php echo $f?>')" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win($f) ?>" data-max-width="100%" data-width="100%"><i class="fa fa-restore"></i></a>
							<?php else: ?>
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php if (FM_PATH == DOMAIN_TRASH_DIR): ?>
							<a title="<?php echo lng('Restore') ?>" onclick="restore('<?php echo FM_PATH?>', '<?php echo $f?>')" href="#" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win($f) ?>" data-max-width="100%" data-width="100%"><i class="fa fa-restore"></i></a>
                        <?php else: ?>
    ...
    <div class="row">
        ...
        <?php else: ?>
            <div class="col-12"><a class="float-right text-muted"><?php echo file_get_contents("../data/logCount"); ?> entries</a></div>
        <?php endif; ?>
    </div>
...
http://stackoverflow.com/a/13821992/1164642
*/
                ...
                <ul class="navbar-nav mr-auto float-right <?php echo fm_get_theme();  ?>">
                            ...
                            <div class="input-group-append btn-group">
                                <span class="input-group-text dropdown-toggle" id="search-addon2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></span>
                                  <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item" id="js-search-modal" data-toggle="modal" data-target="#searchModal">Advanced Search</a>
                                  </div>
                            </div>
                        </div>
                    </li>
                    ...
                    <li class="nav-item avatar dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            ...
                            <?php endif ?>
                            <a title="<?php echo lng('Users & Data') ?>" class="dropdown-item nav-link" href="../admin"><i class="fa fa-users-admin" aria-hidden="true"></i> <?php echo lng('Users & Data') ?></a>
...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.slim.min.js"></script>
...
<head>
    ...
    <link rel="stylesheet" href="http://cdnjs.cloudflare.com/ajax/libs/ekko-lightbox/5.3.0/ekko-lightbox.css" />
    <?php if (FM_USE_HIGHLIGHTJS): ?>
    <link rel="stylesheet" href="http://cdnjs.cloudflare.com/ajax/libs/highlight.js/10.6.0/styles/<?php echo FM_HIGHLIGHTJS_STYLE ?>.min.css">
    <?php endif; ?>
    <style>
        ...
        * { -webkit-border-radius:0 !important;-moz-border-radius:0 !important;border-radius:0 !important; }
        ...
        .fa.fa-restore:before { content: "\2397"; }
        .fa.fa-users-admin:before { content: "\f29a"; }
        
        .fa.fa-browse-domain:before { content:"\235F"; margin-left:-0.5em; margin-right:0.5em; }
...
<script src="http://cdn.datatables.net/plug-ins/1.11.5/sorting/absolute.js"></script>
...
<script>
    ...
    //TFM Config
    window.curi = "http://tinyfilemanager.github.io/config.json", window.config = null;
    ...
    //search
    function fm_search() {
        var searchTxt = $("input#advanced-search").val(), searchWrapper = $("ul#search-wrapper"), _html = ""; $loader = $("div.lds-facebook");
        if(!!searchTxt && searchTxt.length > 1) {
            var data = {ajax: true, content: searchTxt, type: 'search'};
            $.ajax({
                type: "POST",
                url: window.location,
                data: data,
        ...
        } else { searchWrapper.html("OOPS: minimum 2 characters required!"); }
    }
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
        ...
        ?>
    <script src="http://cdnjs.cloudflare.com/ajax/libs/ace/1.4.12/ace.js"></script>
    <script>
